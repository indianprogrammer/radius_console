...
@section('content')
  ...
  <script>
    ...
    function biWirePicker(row) {
      ...
      // Lazy-load the full product list the first time the row is opened
      ...
      // Wrap open() to ensure cache is loaded
      const originalOpen = open;
      const openWithCache = async () => {
        originalOpen();
        await ensureCache();
      };
      toggle.removeEventListener('click', toggle.__handler || (()=>{}));
      toggle.addEventListener('click', e => {
        e.stopPropagation();
        if (popover.hasAttribute('hidden')) openWithCache();
        else close();
      });
    }
