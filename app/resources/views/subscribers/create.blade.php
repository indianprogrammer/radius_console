...
@push('styles')
  <style>
    ...
    /* ══ Outlined text fields with floating labels ═══════════════
       The label rests inside the control and floats up into a notch
       on the top border when the field is focused or filled.       */
    .field:has(.gui-input) {
      position: relative;
      margin-top: .35rem;
    }
    ...
    /* A field with a hint underneath keeps the label anchored to the input,
       not to the whole field box. */
    ...
    /* Float the label: on focus, when filled, or for controls that always
       carry a value (selects, date and file pickers, textareas). */
    ...
    /* Focused field: primary-coloured label and a heavier border. */
    .field:has(.gui-input):focus-within > label:not(.switch-label) {
    ...
    /* Textareas anchor their label to the top border, not the middle. */
    .field:has(textarea) > label:not(.switch-label) { top: 0; }

    /* Fallback for browsers without :has() — keep labels above the inputs. */
